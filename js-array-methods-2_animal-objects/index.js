...
const animalsSortedByWeightStartingWithLowest = animals.slice().sort((a, b) => {
  const animalA = Number(a.weight);
  const animalB = Number(b.weight);
...
const averageWeightOfAllAnimalsInAfrica =
  ...
  animals.slice().filter((animal) => animal.continents.includes("Africa"))
    .length;
