...
– [ ] When the slider control of inputRotation is moved, rotate the box. Hint: You may need to
 work with the CSS property "transform" and a value that contains "rotate" and "deg".
...
inputColor.addEventListener("input", (event) => {
  box.style.background = `hsl(${event.target.value}, 70%, 60%)`;
});

inputRadius.addEventListener("input", (event) => {
  box.style.borderRadius = `${event.target.value * 2}px`;
});
